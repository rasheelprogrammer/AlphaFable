...
if (isset($HTTP_RAW_POST_DATA) && !empty(file_get_contents('php://input'))) {
    ...
    if ($query[1]->num_rows > 0) {
        if ($query[0]->num_rows > 0) {
            if ($result[0]['MaxBankSlots'] < 10) {
                $cost = $NumSlots * 100;
            } else if ($result[0]['MaxBankSlots'] >= 10 && $result[0]['MaxBankSlots'] < 15) {
                $cost = $NumSlots * 300;
            } else if ($result[0]['MaxBankSlots'] >= 15 && $result[0]['MaxBankSlots'] < 35) {
                $cost = $NumSlots * 500;
            }
