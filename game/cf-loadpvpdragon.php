...
if (!empty($HTTP_RAW_POST_DATA)) {
    ...
    if (!empty($xml->intcharID)) {
        ...
        $level = $query[0]['level'] - 10;

        $query[] = $MySQLi->query("SELECT * FROM df_characters WHERE id != '{$charID}' AND level >= '{$level}' AND HasDragon = 1 ORDER BY RAND() LIMIT 1");
        $result[0] = $query[0]->fetch_assoc();
        ...
        if ($query[0]->num_rows != 0) {
            ...
            $query[6] = $MySQLi->query("SELECT * FROM df_class WHERE ClassID = '13'");
