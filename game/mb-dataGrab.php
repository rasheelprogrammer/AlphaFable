                    ...
                    switch (strtolower($_GET['m'])) {
                        case 'quests2':
                            ...
                            for ($intQuestID = $TS; $intQuestID <= $TE; $intQuestID++) {
                                ...
                                if ($items->num_rows == 0) {
                                    ...
                                    $strName = mysql_escape_string($xml->quest["strName"]);
                                    $strDescription = mysql_escape_string($xml->quest["strDescription"]);
                                    $strComplete = mysql_escape_string($xml->quest["strComplete"]);
                                    $strFileName = mysql_escape_string($xml->quest["strFileName"]);
                                    $strXFileName = mysql_escape_string($xml->quest["strXFileName"]);
                                    ...
                                    $strMonsterType = mysql_escape_string($xml->quest["strMonsterType"]);
                                    $strMonsterGroupFileName = mysql_escape_string($xml->quest["strMonsterGroupFileName"]);
                                    ...
                                    if ($TotalMoster > 0) {
                                        for ($a = 0; $a < $TotalMoster; $a++) {
                                            ...
                                            $strCharacterName = mysql_escape_string($xml->quest->monsters[$a]["strCharacterName"]);
                                            ...
                                            $strGender = mysql_escape_string($xml->quest->monsters[$a]["strGender"]);
                                            ...
                                            $strArmorName = mysql_escape_string($xml->quest->monsters[$a]["strArmorName"]);
                                            $strArmorDescription = mysql_escape_string($xml->quest->monsters[$a]["strArmorDescription"]);
                                            $strArmorDesignInfo = mysql_escape_string($xml->quest->monsters[$a]["strArmorDesignInfo"]);
                                            $strArmorResists = mysql_escape_string($xml->quest->monsters[$a]["strArmorResists"]);
                                            ...
                                            $strWeaponName = mysql_escape_string($xml->quest->monsters[$a]["strWeaponName"]);
                                            $strWeaponDescription = mysql_escape_string($xml->quest->monsters[$a]["strWeaponDescription"]);
                                            $strWeaponDesignInfo = mysql_escape_string($xml->quest->monsters[$a]["strWeaponDesignInfo"]);
                                            $strWeaponResists = mysql_escape_string($xml->quest->monsters[$a]["strWeaponResists"]);
                                            $strType = mysql_escape_string($xml->quest->monsters[$a]["strType"]);
                                            ...
                                            $strElement = mysql_escape_string($xml->quest->monsters[$a]["strElement"]);
                                            $strWeaponFile = mysql_escape_string($xml->quest->monsters[$a]["strWeaponFile"]);
                                            $strMovName = mysql_escape_string($xml->quest->monsters[$a]["strMovName"]);
                                            $strMonsterFileName = mysql_escape_string($xml->quest->monsters[$a]["strMonsterFileName"]);
                                            $RaceID = $xml->quest->monsters[$a]["RaceID"];
                                            $strRaceName = mysql_escape_string($xml->quest->monsters[$a]["strRaceName"]);
                                            if ($monids == '') {
                                                $monids = $MonsterID;
                        ...
                        case 'towns': //TEST - DO NOT USER
                            for ($intQuestID = $T2S; $intQuestID <= $T2E; $intQuestID++) {
                                ...
                                $xml = simplexml_load_string($result);
                                $QuestID = $intQuestID;
                                $strFileName = mysql_escape_string($xml->newTown["strQuestFileName"]);
                                $strXFileName = mysql_escape_string($xml->newTown["strQuestXFileName"]);
                        ...
                        case 'class':
                            for ($intClass = $CS; $intClass <= $CE; $intClass++) {
                                ...
                                $strClassName = mysql_escape_string($xml->character["strClassName"]);
                                $strClassFileName = mysql_escape_string($xml->character['strClassFileName']);
                                $strArmorName = mysql_escape_string($xml->character['strArmorName']);
                                $strArmorDescription = mysql_escape_string($xml->character['strArmorDescription']);
                                $strArmorResists = mysql_escape_string($xml->character['strArmorResists']);
                                ...
                                $strWeaponName = mysql_escape_string($xml->character['strWeaponName']);
                                $strWeaponDescription = mysql_escape_string($xml->character['strWeaponDescription']);
                                $strWeaponDesignInfo = mysql_escape_string($xml->character['strWeaponDesignInfo']);
                                $strWeaponResists = mysql_escape_string($xml->character['strWeaponResists']);
                                $intWeaponLevel = $xml->character['intWeaponLevel'];
                                $strWeaponIcon = mysql_escape_string($xml->character['strWeaponIcon']);
                                $strType = mysql_escape_string($xml->character['strType']);
                                $strItemType = mysql_escape_string($xml->character['strItemType']);
                                ...
                                $strElement = mysql_escape_string($xml->character['strElement']);
                        ...
                        case 'shops':
                            for ($intShopID = $SS; $intShopID <= $SE; $intShopID++) {
                                ...
                                $strName = mysql_escape_string($xml->shop["strCharacterName"]);
                                ...
                                if ($TotalItems > 0) {
                                    for ($a = 0; $a < $TotalItems; $a++) {
                                        ...
                                        $i2 = mysql_escape_string($xml->shop->items[$a]["strItemName"]);
                                        ...
                                        $i13 = mysql_escape_string($xml->shop->items[$a]["strItemDescription"]);
                                        $i14 = $xml->shop->items[$a]["bitDragonAmulet"];
                                        $i15 = mysql_escape_string($xml->shop->items[$a]["strEquipSpot"]);
                                        $i16 = mysql_escape_string($xml->shop->items[$a]["strCategory"]);
                                        $i17 = mysql_escape_string($xml->shop->items[$a]["strItemType"]);
                                        $i18 = mysql_escape_string($xml->shop->items[$a]["strType"]);
                                        $i19 = mysql_escape_string($xml->shop->items[$a]["strFileName"]);
                                        ...
                                        $i31 = mysql_escape_string($xml->shop->items[$a]["strResists"]);
                                        $i32 = mysql_escape_string($xml->shop->items[$a]["strElement"]);
                                        $i33 = $xml->shop->items[$a]["intRarity"];
                                        $i34 = $xml->shop->items[$a]["intMaxStackSize"];
                                        $i35 = mysql_escape_string($xml->shop->items[$a]["strIcon"]);
                            ...
                            for ($intShopID = $HS; $intShopID <= $HE; $intShopID++) {
                                ...
                                if ($TotalItems > 0) {
                                    for ($a = 0; $a < $TotalItems; $a++) {
                                        ...
                                        $i2 = mysql_escape_string($xml->shop->sHouses[$a]["strHouseName"]);
                                        $i3 = mysql_escape_string($xml->shop->sHouses[$a]["strHouseDescription"]);
                                        ...
                                        $i20 = mysql_escape_string($xml->shop->sHouses[$a]["strType"]);
                                        $i21 = mysql_escape_string($xml->shop->sHouses[$a]["strIcon"]);
                                        $i22 = mysql_escape_string($xml->shop->sHouses[$a]["strDesignInfo"]);
                                        $i23 = mysql_escape_string($xml->shop->sHouses[$a]["strFileName"]);
                        ...
                        case 'houseitems':
                            for ($intShopID = $HIS; $intShopID <= $HIE; $intShopID++) {
                                ...
                                $strName = mysql_escape_string($xml->houseitemshop["strName"]);
                                $TotalItems = count($xml->houseitemshop->houseitems);
                                $itemList = '';
                                if ($TotalItems > 0) {
                                    for ($a = 0; $a < $TotalItems; $a++) {
                                        $i1 = $xml->houseitemshop->houseitems[$a]["HouseItemID"];
                                        $i2 = mysql_escape_string($xml->houseitemshop->houseitems[$a]["strItemName"]);
                                        $i3 = mysql_escape_string($xml->houseitemshop->houseitems[$a]["strItemDescription"]);
                                        ...
                                        $i21 = mysql_escape_string($xml->houseitemshop->houseitems[$a]["strType"]);
                                        $i22 = mysql_escape_string($xml->houseitemshop->houseitems[$a]["strFileName"]);
                        ...
                        case 'merges':
                            for ($intMergeShop = $MSS; $intMergeShop <= $MSE; $intMergeShop++) {
                                ...
                                $strName = mysql_escape_string($xml->mergeshop["strName"]);
                                ...
                                if ($TotalItems > 0) {
                                    for ($a = 0; $a < $TotalItems; $a++) {
                                        ...
                                        $i2 = mysql_escape_string($xml->mergeshop->items[$a]["strItemName"]);
                                        ...
                                        $i13 = mysql_escape_string($xml->mergeshop->items[$a]["strItemDescription"]);
                                        $i14 = $xml->mergeshop->items[$a]["bitDragonAmulet"];
                                        $i15 = mysql_escape_string($xml->mergeshop->items[$a]["strEquipSpot"]);
                                        $i16 = mysql_escape_string($xml->mergeshop->items[$a]["strCategory"]);
                                        $i17 = mysql_escape_string($xml->mergeshop->items[$a]["strItemType"]);
                                        $i18 = mysql_escape_string($xml->mergeshop->items[$a]["strType"]);
                                        $i19 = mysql_escape_string($xml->mergeshop->items[$a]["strFileName"]);
                                        ...
                                        $i31 = mysql_escape_string($xml->mergeshop->items[$a]["strResists"]);
                                        $i32 = mysql_escape_string($xml->mergeshop->items[$a]["strElement"]);
                                        $i33 = $xml->mergeshop->items[$a]["intRarity"];
                                        $i34 = $xml->mergeshop->items[$a]["intMaxStackSize"];
                                        $i35 = mysql_escape_string($xml->mergeshop->items[$a]["strIcon"]);
                        ...
                        case 'hairs':
                            for ($intHairShop = $HSS; $intHairShop <= $HSE; $intHairShop++) {
                                ...
                                $strName = mysql_escape_string($xml->HairShop["strHairShopName"]);
                                $strFile = mysql_escape_string($xml->HairShop["strFileName"]);
                                $TotalItems = count($xml->HairShop->hair);
                                $itemList = '';
                                if ($TotalItems > 0) {
                                    for ($a = 0; $a < $TotalItems; $a++) {
                                        $i1 = $xml->HairShop->hair[$a]["HairID"];
                                        $i2 = mysql_escape_string($xml->HairShop->hair[$a]["strName"]);
                                        $i3 = mysql_escape_string($xml->HairShop->hair[$a]["strFileName"]);
                                        $i8 = $xml->HairShop->hair[$a]["intFrame"];
                                        $i6 = $xml->HairShop->hair[$a]["RaceID"];
                                        $i4 = $xml->HairShop->hair[$a]["intPrice"];
                                        $i5 = mysql_escape_string($xml->HairShop->hair[$a]["strGender"]);
                                    ...
                                    if ($items->num_rows == 0) {
                                        ...
                                        if ($TotalItems > 0) {
                                            for ($a = 0; $a < $TotalItems; $a++) {
                                                ...
                                                $i2 = mysql_escape_string($xml->character->items[$a]["strItemName"]);
                                                ...
                                                $i13 = mysql_escape_string($xml->character->items[$a]["strItemDescription"]);
                                                $i14 = $xml->character->items[$a]["bitDragonAmulet"];
                                                $i15 = mysql_escape_string($xml->character->items[$a]["strEquipSpot"]);
                                                $i16 = mysql_escape_string($xml->character->items[$a]["strCategory"]);
                                                $i17 = mysql_escape_string($xml->character->items[$a]["strItemType"]);
                                                $i18 = mysql_escape_string($xml->character->items[$a]["strType"]);
                                                $i19 = mysql_escape_string($xml->character->items[$a]["strFileName"]);
                                                ...
                                                $i31 = mysql_escape_string($xml->character->items[$a]["strResists"]);
                                                $i32 = mysql_escape_string($xml->character->items[$a]["strElement"]);
                                                $i33 = $xml->character->items[$a]["intRarity"];
                                                $i34 = $xml->character->items[$a]["intMaxStackSize"];
                                                $i35 = mysql_escape_string($xml->character->items[$a]["strIcon"]);
